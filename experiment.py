...
from evaluation.evaluate_episodes import evaluate_episode, evaluate_causal_episode
from models import DecisionTransformer, CausalDecisionTransformerV1, CausalDecisionTransformerV2
from trainers import SequenceTrainer, CausalSequenceTrainer
from gridworld import Craft
# from dm_alchemy import symbolic_alchemy

GRIDWORLD_FOLDER_PATH = "./gridworld/maps"
...
def experiment(
    ...
    if env_name == 'alchemy':
        ...
        env_targets = []
        scale = 100.
    elif env_name == 'craft' or env_name == 'craft-causal':
        max_ep_len = 100
        seed = 2022
        rng = random.Random(seed)
        env_targets = [0, 1, 2, 3, 4]
        scale = 1.
        map_file_path = f"{GRIDWORLD_FOLDER_PATH}/fourobjects.txt"

        if env_name == 'craft':
            env = Craft(map_file_path, rng, causal=False)
        else:
            env = Craft(map_file_path, rng, causal=True)
            ...
            for _ in tqdm(range(num_eval_episodes)):
                with torch.no_grad():
                    if causal_dim is None:
                        ...
                        ret, length = evaluate_causal_episode(
                            env,
                            state_dim,
                            act_dim,
                            causal_dim,
    ...
    for iter in range(variant['max_iters']):
        ...
        CHECKPOINTS_PATH = f'checkpoints/{env}-{dataset}-{iter}.pt'
